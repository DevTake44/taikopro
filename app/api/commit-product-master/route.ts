...
import { PRODUCT_MASTER_CONFLICT_COLUMNS } from "@/lib/uploadRules";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rows = body?.rows as ProductMasterRow[] | undefined;
    ...
    const { error } = await supabase
      .from("product_master")
      .upsert(payload, { onConflict: PRODUCT_MASTER_CONFLICT_COLUMNS });
    if (error) {
